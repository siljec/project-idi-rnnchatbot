...
def embedding_attention_seq2seq(encoder_inputs,
  ...
  with variable_scope.variable_scope(
    ...
    if isinstance(feed_previous, bool):
      outputs, state, beam_path, beam_symbol = embedding_attention_decoder(
        decoder_inputs,
        encoder_state,
        attention_states,
        cell,
        ...
        beam_size=beam_size)
      return outputs, state, encoder_state, beam_path, beam_symbol
...
def model_with_buckets(encoder_inputs, decoder_inputs, targets, weights,
  ...
  with ops.name_scope(name, "model_with_buckets", all_inputs):
    for j, bucket in enumerate(buckets):
      with variable_scope.variable_scope(variable_scope.get_variable_scope(),
        ...
        bucket_outputs, _, bucket_states, beam_paths, beam_symbols = seq2seq(encoder_inputs[:bucket[0]],
                                    decoder_inputs[:bucket[1]])
        outputs.append(bucket_outputs)
        state = bucket_states
        beam_paths.append(beam_paths)
        beam_symbols.append(beam_symbols)
        if per_example_loss:
  ...
  return outputs, losses, state, beam_paths, beam_symbols
