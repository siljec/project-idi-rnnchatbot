...
def preprocessing2(x_path, y_path, fast_text_training_file_path, folder, force_create_new_files, force_train_fast_model_all_over = False):
    ...
    if path_exists("./"+str(folder)+"/model.bin") and not force_train_fast_model_all_over:
        print("Load existing FastText model...")
        model = fasttext.load_model('./'+str(folder)+'/model.bin', encoding='utf-8')
    else:
