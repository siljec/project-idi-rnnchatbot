...
def generate_all_files(vocab_size=1000, tokens=["_PAD", "_GO ", "_EOS", "_EOT", "_UNK"], val_size_fraction=0.1,
    ...
    print('Get vectors for vocabulary words')
    with open(vocabulary_path) as vocabObject:
        vocab_words = {}
        for word in vocabObject:
            vocab_words[word] = model[word]
    ...
    for unk_key, unk_values in unknown_words.iteritems():
        min_dist = 1
        word = ""
        print(unk_key, unk_values)
        for key, value in vocab_words.iteritems():
            cur_dist = spatial.distance.cosine(unk_values, value)
            if cur_dist < min_dist:
                min_dist = cur_dist
                word = key
        unknown_words[unk_key] = word

    print('Replace unknown words with similar words')
    counter = 0
